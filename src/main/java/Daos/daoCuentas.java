...
public class daoCuentas implements inCuentas {
	...
	private final String obtenerIdCuenta = "Select AUTO_INCREMENT from information_schema.TABLES where TABLE_SCHEMA='bancoparcial' and TABLE_NAME='cuentas';";
	...
	private PreparedStatement valoresQuery(Connection cn, String query, Cuenta cuenta) {
		...
		try {
			...
			if (query.equals(Agregar)) {
				...
				ps.setInt(4, cuenta.getNroCuenta());
	...
	public String obtenerTipoCuentaMasCreadaEnRango(Date desde, Date hasta) {
		...
		try {
			...
			System.out.println(
					"DEBUG DAO: Ejecutando consulta para tipo de cuenta más creado. Fechas: " + desde + " a " + hasta);
